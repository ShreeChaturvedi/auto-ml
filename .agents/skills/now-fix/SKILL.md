...
If the `$issue` step didn't fully explore the relevant code, do so now. Search and read files to understand the current state of the code that needs to change.
...
Make the changes. Follow the project's coding conventions (see AGENTS.md). Keep changes focused — fix the issue, don't refactor unrelated code.
