...
Don't just read the diff — read each changed file in full so you understand the context around the changes. This is essential for an informed review.
...
- **Quality verdict:** one of:
  ...
  - **Acceptable** — minor issues that can be cleaned up post-merge
...
Then review the merged code for any cleanup opportunities — duplicate code, dead imports, naming inconsistencies. Make fixes as a separate commit.

Scale the cleanup to the MR size: small MRs need a quick scan, large MRs need thorough review.
...
- If merged: what cleanup was done (if anything)
