...
export function buildEvaluationScript(options: BuildEvaluationScriptOptions): string {
  ...
  lines.push('fi = {}');
  lines.push('');
  lines.push('# Resolve feature names from the trained pipeline (handles OHE expansion)');
  lines.push('if hasattr(pipeline, "feature_names_in_"):');
  lines.push('    _fi_columns = list(pipeline.feature_names_in_)');
  lines.push('elif hasattr(pipeline, "named_steps") and hasattr(pipeline.named_steps.get("preprocessor", None), "get_feature_names_out"):');
  lines.push('    _fi_columns = list(pipeline.named_steps["preprocessor"].get_feature_names_out())');
  lines.push('else:');
  lines.push('    _fi_columns = feature_columns');
  lines.push('');
  ...
  lines.push('        "features": _fi_columns,');
