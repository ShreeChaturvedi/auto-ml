...
export function extractLatestStepNotebookContext(state: WorkflowGraphState): StepNotebookContext | null {
  ...
  for (let index = state.toolResultHistory.length - 1; index >= 0; index -= 1) {
    ...
    const cellIds = Array.isArray(step?.cellIds)
      ? step.cellIds.filter((value): value is string => typeof value === 'string')
      : [];
...
export function extractLatestRunCellContext(toolResults: ToolResult[]): LatestRunCellContext | null {
  for (let index = toolResults.length - 1; index >= 0; index -= 1) {
    const result = toolResults[index];
    ...
    const output = asRecord(result.output);
    return {
      cellId: extractLatestCellId([result]),
