...
//
// Two fixture styles coexist:
//  1. The legacy `NotebookCellFixture[]` exports feed the landing-local
//     `<NotebookColumn>` used by FeatureEngineering + Training.
//  2. `preprocessingNotebookCells` below uses the real frontend `RichOutput`
//     shape so the rebuilt Preprocessing tab can render its outputs through
//     the real `<NotebookCellOutput>` leaf component.
...
//
// NotebookCellOutput expects RichOutput[] (see @frontend/lib/api/execution).
// We NEVER emit `type: 'chart'` here because that branch lazy-loads Plotly
// (~4.9 MB) in NotebookCellOutput → CellOutputRenderer → PlotlyOutput. The
// landing page is bundle-lean: text + table only.
// ---------------------------------------------------------------------------
...
//
// Parallel to `trainingNotebookCells` above. Consumed by the rebuilt
// <FeatureEngineeringView> via the real frontend <NotebookCellOutput>.
