...
/**
 * Video-workspace Tailwind config.
 *
 * Mirrors `frontend/tailwind.config.js` verbatim (theme extension + plugins)
 * so real React components imported from `frontend/src/` render with their
 * intended styles inside Remotion compositions.
 *
 * Content globs also include `frontend/src/**` so Tailwind discovers classes
 * used by the real components we pull in through webpack aliases.
 *
 * NOTE: `video/AGENTS.md` line 173-175 previously advised against top-level
 * Tailwind in this workspace — the user explicitly overrode that guidance
 * for the agentic-demo composition so the real app CSS renders verbatim.
 */
const config: Config = {
  darkMode: "class",
  content: [
    "./remotion/**/*.{ts,tsx}",
    "./src/frontend-bridge/**/*.{ts,tsx}",
    "../frontend/src/**/*.{ts,tsx}",
  ],
  theme: {
    extend: {
      ...
      animation: {
        ...
        "timeline-skeleton-sweep":
          "timeline-skeleton-sweep 1.5s ease-in-out infinite",
        "skeleton-shimmer":
          "timeline-skeleton-sweep 1.5s ease-in-out infinite",
      },
    },
